...
class SnailNum {
    ...
    simplify() {
        return [...this.arr.map( el => isNaN(el) ? el.simplify() : el)]
...
function log(num) {
    console.log(stringify(num.simplify(), {maxLength: 400, indent: ' '}))
}
...
nums.forEach((num1, i) => {
    nums.forEach((num2, j) => {
        ...
        const sum = add(toSnailNum(num1), toSnailNum(num2))
        let depthViolation = sum.getDepthViolations()
        let valueViolation = depthViolation.length ? [] : sum.getValueViolations()
        while (depthViolation.length || valueViolation.length) {
            reduce(sum, depthViolation, valueViolation)
            depthViolation = sum.getDepthViolations()
            valueViolation = depthViolation.length ? [] : sum.getValueViolations()
        }
        allSums.push(sum.getMagnitude())
    })
})
