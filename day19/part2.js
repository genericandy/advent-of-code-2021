...
function calcDistance(originA, originB) {
    return originA.reduce((sum, posA, i) => sum + Math.abs(posA - originB[i]), 0)
}

function findLongestDistance() {
    const distances = []
    sensors.forEach((sensorA, a) => {
        sensors.slice(a + 1).forEach((sensorB) => {

            distances.push(calcDistance(sensorA.origin, sensorB.origin))
        })
    })
